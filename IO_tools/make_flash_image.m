...
for j = 1:num_patterns
    temploadfile = load([file_list(j).PathName '\' file_list(j).FileName]); %DJB: This is the fix for the pattern.num_panels error (7.2021)
    pattern = temploadfile.pattern;
...
% determine if row_compression is on
    ...
    if (row_compression(j))
        ...
        current_frame_size(j) = (pattern.num_panels(1))*pattern.gs_val*8;
    end
    ...
    if (row_compression(j)) == 1 % hack - append 10 to gs_val to let controller know that it is a row_compressed pattern w/o adding more to header block
        ...
        Header_block(1:8) = [dec2char(pattern.x_num,2), dec2char(pattern.y_num,2), pattern.num_panels(1), pattern.gs_val, dec2char(current_frame_size(j),2)];
    end
    % set up SD structure with pattern info
    SD.x_num(j) = pattern.x_num;
    SD.y_num(j) = pattern.y_num;
    SD.num_panels(j) = pattern.num_panels(1);
    SD.gs_val(j) = pattern.gs_val; % unclear if we should change this to reflect 11, 12, 13 hack
    SD.frame_size(j) = current_frame_size(1);
    ...
    % ben 9/28: override incorrect temp path (no longer installed at (x86)
    % folder
%     temp_path = 'C:\pats_temp';
